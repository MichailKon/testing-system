...
func (i *Invoker) Compile(tester *JobExecutor, job *Job) {
	...
	if j.compileResult.Err != nil {
		logger.Error("Can not compile submit %d in job %s error: %s", job.Submission.ID, job.ID, j.compileResult.Err.Error())
		j.invoker.FailJob(job, "can not compile submit in job %s, error: %s", job.ID, j.compileResult.Err.Error())
		return
	}
	logger.Trace("Finished compilation process of submit %d in job %s with verdict %v", job.Submission.ID, job.ID, j.compileResult.Verdict)
	...
	if err == nil {
		...
		logger.Error("Compilation of submit %d in job %s send result error: %s", job.Submission.ID, job.ID, err.Error())
		j.invoker.FailJob(job, "can not upload compilation result of submit %d, job %s, error: %s", job.Submission.ID, job.ID, err.Error())
	}
}
...
func (j *compileJob) Finish() error {
	...
	case verdict.TL:
		j.compileResult.Verdict = verdict.CE
		outputReader = strings.NewReader(fmt.Sprintf("Compilation took more than %v time", j.language.Limits.TimeLimit))
	case verdict.ML:
		j.compileResult.Verdict = verdict.CE
		outputReader = strings.NewReader(fmt.Sprintf("Compilation took more than %v memory", j.language.Limits.MemoryLimit))
	case verdict.WL:
		j.compileResult.Verdict = verdict.CE
		outputReader = strings.NewReader(fmt.Sprintf("Compilation took more than %v wall time", j.language.Limits.WallTimeLimit))
	case verdict.SE:
		j.compileResult.Verdict = verdict.CE
